...
| Linear probability summary | Screen-reader sentence inside the atomic live region |
| Structured result details | Definition list |
| Visible focus styling | Theme-aware three-pixel outline with offset |
...
Automated tests cannot evidence the behaviour generated by the browser and Gradio around that markup, so the manual checks below are required. They were performed on 24 September 2026 in Windows with Microsoft Edge 153, Narrator and Lighthouse; results are recorded in the accessibility checklist. Repeat them after any Gradio upgrade or interface change.
...
Fresh-clone installation rehearsal, manual qualitative error coding and screenshots are outside the final committed scope unless the owner later chooses to perform and record them. Browser-dependent accessibility checks were performed on 24 September 2026 and are recorded in the accessibility checklist.
